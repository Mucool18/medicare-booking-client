...
import {BASE_URL, USER_IMG_PATH, token} from '../../config.js';
...
const Profile = ({user}) => {
  ...
  useEffect(()=>{
    setFormData({name: user.name, email: user.email, photo: user.photo, bloodType: user.bloodType, gender: user.gender})
  },[])
  ...
  return (
    <div className="mt-10">
      <form onSubmit={submitHandler}>
        ...
        <div className="mb-5 flex items-center gap-3">
          {formData.photo &&
            <figure className="w-[60px] h-[60px] rounded-full border-2 border-solid border-primaryColor flex items-center justify-center">
              <img src={`${USER_IMG_PATH}/${formData.photo}`} alt="" className="w-full rounded-full" />
            </figure>
          }
        ...
        <div className="mt-7">
          <button disabled={loading && true} type="submit" className="w-full  bg-primaryColor text-white text-[18px] leading-[30px] rounded-lg py-3 px-4 ">{loading ? <HashLoader size={25} color='#ffffff' /> : "Update"}</button>
        </div>
      </form>
    </div>
  )
